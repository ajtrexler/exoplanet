# -*- coding: utf-8 -*-
"""
Created on Sat Sep 17 17:01:06 2016
...
plt.savefig('exo_figure1.png')
plt.show()


#next goal is to identify any structure in star system or orbital parameters.
#take star and planetary numeric data, impute nan values using KDE.
#KDE generation of NaN values is just for fun at this level, as it relies on the 
#fact that KDE is gaussian in each case.
...
missing_sys=pd.DataFrame(index=sys_cols,columns=['missing_vals_est','kde'])
missing_orbitals=pd.DataFrame(index=orbit_cols,columns=['missing_vals_est','kde'])   

#detect how many missing values per column in the df
for s,o in zip(sys_cols,orbit_cols):
    missing_sys.loc[s,'missing_vals_est']=len(oec)-np.sum(pd.notnull(oec[s]))
    missing_orbitals.loc[o,'missing_vals_est']=len(oec)-np.sum(pd.notnull(oec[o]))
...
#now with filled values want to PCA them.  probably want to scale and normalize first though.
#may also want to KNN the pca object to pull out clustering.

#also plot the destination indices onto these things.  systems look like there's some structure in the data.
...
oec_sys_pca=pca_obj.fit_transform(scaled_sys)
plt.scatter(x=oec_sys_pca[:,0],y=oec_sys_pca[:,1],s=5)
plt.scatter(oec_sys_pca[destinations.index.values,0],oec_sys_pca[destinations.index.values,1],c='r')
plt.xlim([-5,5])
plt.ylim([-5,5])
...
#how do we get there?
#how big is the generation ship.
